...
pattern = r'a\\b\tc'
pattern = r'\w+$'
# тут какие символы отфилтровать и откуда начать и чтобы все символы были вместе или раздельно

print(re.findall(pattern, s, flags=re.IGNORECASE))
# это флаг он делает все данные нижнего регистра
def validate_email(email):
    return re.match(r'^.+@(\w+\.){0,2}[a-z]{2,6}$', email, re.IGNORECASE)


print(validate_email('mail@mail.com'))
# это проверка строки на правильность написания
